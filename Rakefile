...
task :s3 do
  ...
  puts "## Checking for AWS variables ..."
  aws_variables = %w{AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_REGION}
  missing = aws_variables.find_all { |v| ENV[v] == nil }
  unless missing.empty?
    raise "The following variables are missing and are needed to run this script: #{missing.join(', ')}."
  end

  puts "## Uploading ..."
  uploader = S3FolderUpload.new(public_dir, 'chr.ishenry.com')
  uploader.upload!
...
class S3FolderUpload
  ...
  def initialize(folder_path, bucket)
    ...
    resp.buckets.each do |list_bucket|
      if list_bucket.name == bucket
        @s3_bucket = list_bucket
        return
      end
    end
  end
    ...
    thread_count.times do |i|
      threads[i] = Thread.new {
        until files.empty?
          ...
          key = file_name.gsub('public/', '')

          File.open(file_name, 'rb') do |file|
            @connection.put_object(bucket: s3_bucket.name, key: key, body: file)
          end
        end
      }
    end
